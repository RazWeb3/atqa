...
実際の音声では、表示本文の `IT` が「イット」と読まれることがある。正しい読み辞書は「アイティー」であるため、期待読みと実際に聞こえた読みを比較し、この誤読候補を実音声QAで検出する必要がある。読み上げ原稿が正しくてもTTSエンジンが誤読する場合があるため、読み上げ原稿QAだけでは完了としない。
...
期待読み：あいてぃーぷろじぇくと
聞こえた読み：いっとぷろじぇくと
結果：AUDIO_PRONUNCIATION_SUSPECT
```
...
- `SQLを実行する` と「シークエルを実行する」の差分を検出する。
- 一致する読み上げ原稿を正常とする。
- `synthesisText` 欠損時にStage 1をスキップする。

### 20.4 実音声QA

- `IT` を「イット」と読む音声を `AUDIO_PRONUNCIATION_SUSPECT` として検出する。
- `IT` を「アイティー」と読む音声を正常とする。
- STTが `IT` と表記補正しても、Geminiが「イット」と聞き取った場合は要確認にする。
- STTとGeminiが競合し、Geminiに時間・聞こえた読みの根拠がない場合は判定不能にする。
...
- 両形式で連続再生順が正しい。
- `doc-1` の実音声で `IT` を「イット」と読む箇所を誤読候補として検出できる。
- 選択音声をGoogle基盤で検査できる。
- 要確認結果から該当位置を再生できる。
...
6. 実音声をSTTとGeminiで検査する。
7. 「イット」と聞こえる箇所をStage 2の誤読候補として表示する。
8. 要確認箇所から再生する。
